    ...
    <script>
        ...
        async function initCamera() {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                showFallback('您的瀏覽器不支援即時攝影，請上傳照片。');
                return;
            }
            ...
            try {
                ...
                function sendFrame() {
                    ...
                    canvas.toBlob(blob => {
                        fetch('/upload', { method: 'POST', body: blob }).catch(e => info.textContent = '連接中斷: ' + e.message);
                    }, 'image/jpeg', 0.7);
        ...
        sendPhoto.addEventListener('click', () => {
            ...
            info.textContent = '上傳照片中...';
            fetch('/upload', { method: 'POST', body: file })
                .then(() => { info.textContent = '照片已上傳。請重新選擇或等待電腦畫面更新。'; })
                .catch(err => { info.textContent = '上傳失敗: ' + err.message; });
        });
